		...
		if ( $product && taxonomy_exists( $attribute ) ) {
			...
			foreach ( $terms as $term ) {
				if ( in_array( $term->slug, $options, true ) ) {
					...
					// $attribute_priceper_gram ="";
					// //satvinderjit--------
					// if(is_single()){
					// 	$attribute_priceper_gram = assign_per_gram_attriute_price($term->slug);					 
					// }
					...
					// if($attribute_priceper_gram){
					// 	$data .= '<div class="price-n-gram"><div class="attr-price">'.$attribute_priceper_gram['display_price'].'</div><div class="price_per_gm">'.$attribute_priceper_gram['pergram_price'].'/gm </div></div>';
					// }
...
function assign_per_gram_attriute_price( $attribute_key ) {

	$variable_products_price = get_all_variations();

	if( array_key_exists( $attribute_key, $variable_products_price ) ) {


		return $variable_products_price[$attribute_key];
...
function get_all_variations() {
	...
	foreach ($available_variations as $key => $available_variation) {
		...
		$attribute_name = $available_variation['attributes']['attribute_pa_weight'];
		$is_weight = 1;

		if(isset($available_variation['attributes']['attribute_pa_amount'])){

			$attribute_name = $available_variation['attributes']['attribute_pa_amount'];
			$is_weight = 0;
		}
		...
		$pergram_price = get_pergram_price($weight, $available_variation['display_price'], $is_weight);


		//echo $pergram_price."==========".$weight."--------".$available_variation['display_price']."<br>";
	
		$variable_products_price[$attribute_name] = ['pergram_price' => wc_price($pergram_price), 'display_price' => wc_price($available_variation['display_price'])];

	
	}


	return $variable_products_price;
