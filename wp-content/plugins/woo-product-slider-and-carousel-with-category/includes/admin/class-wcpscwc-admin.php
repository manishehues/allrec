...
class Wcpscwc_Admin {
	...
	function wcpscwc_admin_processes() {
		...
		if( $pagenow == 'admin.php' ) {
			...
			if( $current_page == 'wcpscwc-upgrade-pro' ) {
				...
				wp_redirect( WCPSCW_PLUGIN_LINK_UPGRADE );
				exit;
			}

			if( $current_page == 'wcpscwc-bundle-deal' ) {

				wp_redirect( WCPSCW_PLUGIN_BUNDLE_LINK );
				exit;
			}
		}
	}
	...
	function wcpscwc_register_menu() {
		...
		//add_submenu_page( 'wcpscwc-about', __('Join $0 Pro Trail - Woo Product Slider', 'woo-product-slider-and-carousel-with-category'), '<span style="color:#ff2700">'.__('Join $0 Pro Trail', 'woo-product-slider-and-carousel-with-category').'</span>', 'edit_posts', 'wcpscwc-premium', array($this, 'wcpscwc_premium_page') );
	...
	// 	include_once( WCPSCWC_DIR . '/includes/admin/settings/trail-premium.php' );
	// }
